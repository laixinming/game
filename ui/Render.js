...
export class Render {
  ...
  static nftList(){
    ...
    let html = "";
    if(list.length === 0){
      html = '<p style="color:#888">去战斗获取装备</p>';
    }else{
      list.forEach(n => {
        const level = enhance.level(n.tokenId);
        const isEquipped = p.equip[n.type] == n.tokenId;

        html += `
        <div class="card" style="border-color:${getQualityColor(n.quality)}">
          <h3 style="color:${getQualityColor(n.quality)}">${n.name}+${level}</h3>
        ...
        html += `
          ...
          <button style="background:#ff4444; color:white; border:none; border-radius:6px;" onclick="game.render.destroy('${n.tokenId}')">分解</button>
        </div>
        `;
      });
    }
  ...
  static destroy = debounce((tokenId) => {
    if(!confirm("确定分解装备？返还100金币")) return;
